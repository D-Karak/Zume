...
export async function generateSummary( {data}: {data:genSummaryPropValues} ): Promise<string> {
  const response = await ai.models.generateContent({
    ...
    contents: `Write one concise, professional and short resume summary for ${data.fistName} ${data.lastName}, a ${data.jobTitle} based in ${data.city}, ${data.country}.20 words maximum of 35 words(only one)`,
  });
